...
| `registries` | Registries to import when the project has no `supply-chain/` directory. Comma-separated known names or `name=url`. Ignored when `supply-chain/` exists. | `google, mozilla, bytecodealliance` | `mozilla,acme=https://acme.example/audits.toml` |
